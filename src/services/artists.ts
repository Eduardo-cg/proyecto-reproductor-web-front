...
export const getArtists = async (page: number = 0, size: number = 20, search: string = ''): Promise<ArtistsResult> => {
...
export const getArtistsList = async (page: number = 0, size: number = 10, search: string = ''): Promise<ArtistDTO[]> => {
  const params = new URLSearchParams({ page: String(page), size: String(size) })
  if (search) params.set('search', search)
  const res = await fetch(`${API_URL}/artists/list?${params}`, { headers: authHeaders() })
  return handleResponse<ArtistDTO[]>(res, 'Error al obtener la lista de artistas')
}

export const getAlbumsList = async (artistIds: number[] = [], page: number = 0, size: number = 10, search: string = ''): Promise<AlbumDTO[]> => {
  const params = new URLSearchParams({ page: String(page), size: String(size) })
  if (search) params.set('search', search)
  ...
  return handleResponse<AlbumDTO[]>(res, 'Error al obtener la lista de álbumes')
}
...
export const getArtistTracks = async (id: number, page: number = 0, size: number = 20): Promise<ArtistTracksResult> => {
...
export const getArtistAlbums = async (id: number, page: number = 0, size: number = 20): Promise<ArtistAlbumsResult> => {
